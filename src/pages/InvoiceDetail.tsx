...
export function InvoiceDetail() {
  ...
  function invoiceMarkup(includeActions = false) {
    return invoiceRef.current?.querySelector('[data-invoice-document]') as HTMLElement | null || (includeActions ? invoiceRef.current : null)
  }

  async function createPdf(): Promise<jsPDF | null> {
    let host: HTMLDivElement | null = null
    try {
      const source = invoiceMarkup()
      ...
      clone.removeAttribute('data-invoice-document')
      ...
      if (table) {
        ...
        if (colgroup) colgroup.style.display = 'table-column-group'
      }
      ...
      if (renderedHeight <= height) {
        pdf.addImage(canvas.toDataURL('image/jpeg', 0.95), 'JPEG', margin, margin, width, renderedHeight, undefined, 'FAST')
        return pdf
      }
      ...
      while (y < canvas.height) {
        ...
        const h = Math.min(cropHeight, canvas.height - y)
        ...
        pdf.addImage(pageCanvas.toDataURL('image/jpeg', 0.95), 'JPEG', margin, margin, width, h * scale, undefined, 'FAST')
        y += h
        page += 1
      }
      return pdf
    ...
    <div className="min-h-screen pb-8 bg-[color:var(--color-paper)]">
      <header className="sticky top-0 z-20 bg-[color:var(--color-paper)]/95 backdrop-blur border-b border-[#E7E2D6]">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-slate-500">
            <Link to={`/invoices/${invoice.id}/edit`}><Pencil size={18} /></Link>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4">
            ...
            <div className="flex justify-between gap-4 mb-5">
              ...
              <div className="text-xs shrink-0">
                <div className="grid grid-cols-[auto_auto] gap-x-4 gap-y-1">
                  <span className="font-bold text-slate-900">INVOICE #</span><span className="text-right text-slate-500 font-mono-tab">{invoice.invoice_number}</span>
                  <span className="font-bold text-slate-900">DATE</span><span className="text-right text-slate-500">{format(parseISO(invoice.issue_date), 'dd/MM/yyyy')}</span>
                  <span className="font-bold text-slate-900">DUE DATE</span><span className="text-right text-slate-500">{format(parseISO(invoice.due_date), 'dd/MM/yyyy')}</span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full border-collapse table-fixed" style={{ tableLayout: 'fixed' }}>
                ...
                <thead>
                  <tr className="bg-[color:var(--color-ledger)] text-white text-[10px] sm:text-[11px] font-semibold">
                    <th className="text-left px-2 sm:px-3 py-2 font-semibold">Description</th>
                    <th className="text-center px-1 py-2 font-semibold">QTY</th>
                    <th className="text-right px-1.5 py-2 font-semibold">Price</th>
                    <th className="text-right px-2 sm:px-3 py-2 font-semibold">Amount</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, i) => (
                    <tr key={item.id} className={i % 2 === 1 ? 'bg-[#F5F7FB]' : 'bg-white'}>
                      <td data-description className="px-2 sm:px-3 py-2 text-[10px] sm:text-xs align-top whitespace-normal break-words leading-snug">{item.description}</td>
                      <td data-number className="px-1 py-2 text-[10px] sm:text-xs font-mono-tab text-center text-slate-600 align-top whitespace-nowrap">{item.quantity}</td>
                      <td data-number className="px-1.5 py-2 text-[10px] sm:text-xs font-mono-tab text-right text-slate-600 align-top whitespace-nowrap">{formatMoney(item.rate)}</td>
                      <td data-number className="px-2 sm:px-3 py-2 text-[10px] sm:text-xs font-mono-tab text-right align-top whitespace-nowrap">{formatMoney(item.quantity * item.rate)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center justify-between mt-0.5"><p className="text-base font-medium">{invoice.client?.name}</p>{invoice.status === 'draft' && <span className="rounded-full bg-[#EFEFEF] text-slate-500 text-xs font-medium px-2.5 py-1">Not Sent</span>}</div>
          {balance > 0 && balance < total && <p className="text-sm text-[color:var(--color-bad)] font-medium mt-0.5">{formatMoney(balance)} unpaid</p>}
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-5">
          <button onClick={downloadPdf} className="flex flex-col items-center gap-1.5 py-2 text-xs font-medium text-slate-700"><span className="w-11 h-11 rounded-full bg-[color:var(--color-ledger-dim)] flex items-center justify-center text-[color:var(--color-ledger)]"><Download size={18} /></span>Download</button>
          <button onClick={() => window.print()} className="flex flex-col items-center gap-1.5 py-2 text-xs font-medium text-slate-700"><span className="w-11 h-11 rounded-full bg-[color:var(--color-ledger-dim)] flex items-center justify-center text-[color:var(--color-ledger)]"><Printer size={18} /></span>Print</button>
          <Link to={`/invoices/${invoice.id}/edit`} className="flex flex-col items-center gap-1.5 py-2 text-xs font-medium text-slate-700"><span className="w-11 h-11 rounded-full bg-[color:var(--color-ledger-dim)] flex items-center justify-center text-[color:var(--color-ledger)]"><Pencil size={18} /></span>Edit</Link>
          <div className="relative"><button onClick={() => setShowMore(v => !v)} className="flex flex-col items-center gap-1.5 py-2 text-xs font-medium text-slate-700 w-full"><span className="w-11 h-11 rounded-full bg-[color:var(--color-ledger-dim)] flex items-center justify-center text-[color:var(--color-ledger)]"><MoreHorizontal size={18} /></span>More</button>{showMore && <div className="absolute right-0 top-full mt-1 z-10 bg-white border border-[#E7E2D6] rounded-lg shadow-lg py-1 w-36"><button onClick={handleDelete} className="w-full px-3 py-2 text-left text-xs font-medium text-[color:var(--color-bad)] flex items-center gap-2 hover:bg-[color:var(--color-bad-dim)]"><Trash2 size={14} />Delete invoice</button></div>}</div>
        </div>
      ...
      <style>{`@media print { body { background: #fff !important; } header, body > div > div > div:not(.print-invoice) { } }`}</style>
    </div>
  )
}
