...
export function InvoiceDetail() {
  ...
  async function createPdf(): Promise<jsPDF | null> {
    ...
    try {
       ...
       * Amount
       *
       * We explicitly protect the last three columns so
       * financial values cannot overlap the description.
       */
      ...
      for (const image of images) {
        try {
          ...
          /*
           * Image errors must not stop PDF creation.
           */
        }
      }
  ...
  async function downloadPdf() {
     ...
     *
     * This prevents TypeScript from complaining that invoice
     * may become null inside the async function.
     */
    ...
    try {
      ...
      const fileName =
        `${currentInvoice.invoice_number}.pdf`
  ...
  async function handleSend() {
    /*
     * Stable invoice reference for async operations.
     */
        ...
        if (
          ...
          try {
            await navigator.share({
              ...
              text:
                `Invoice ${currentInvoice.invoice_number} from ${
                  meta.business_name ||
                  'your business'
                } — ${formatMoney(total)}.`,
            ...
            {items.map(
              (item, i) => (
                ...
                >
                  ...
                  <span className="font-mono-tab text-center text-slate-600 whitespace-nowrap">
                    {
                      item.quantity
                    }
                  </span>
